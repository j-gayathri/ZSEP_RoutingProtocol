...
% m value in other alg is 0.2 and 0.8 in improved
...
%    r
    ...
    for i=1:1:40     %   z2 - 1
        ...
        if (S2(i).E<=0)            
			...
			temp_rand=rand;
			if(CH(j)==0 && S2(i).type=='A' &&( temp_rand <= ( padv / ( 1 - padv * mod(r,round(1/padv)) )) ))
			    %maxEnergy=S2(i).E;
				CH(j)=i;				
			end
        end
    ...
    for i=1:1:8	   
		if(CH(i)>0)
			...
			if (distance>do)
				S2(CH(i)).E=S2(i).E- ( (ETX+EDA)*(4000) + Emp*4000*( distance*distance*distance*distance ));
			end
			if (distance<=do)
				S2(CH(i)).E=S2(i).E- ( (ETX+EDA)*(4000)  + Efs*4000*( distance * distance ));
			end
        end
    end
	...
	for i=1:1:40
		...
		if(S2(i).E >0 && S2(i).type=='A')
		
			if(CH(j)>0)   % if there is a  cluster then dist betn CH and adv node
				distance=sqrt( (S2(i).xd-S2(CH(j)).xd )^2 + (S2(i).yd-S2(CH(j)).yd )^2 );
			else  			%if no CH then dist betn adv node and BS
				distance=sqrt( (S2(i).xd-(S2(n+1).xd) )^2 + (S2(i).yd-(S2(n+1).yd) )^2 );
			end
...
figure(2);
r=0:rmax;
plot(r,STATISTICS.ALLIVE3(r+1),'-b',r,STATISTICS.ALLIVE(r+1),'-r',r,STATISTICS.ALLIVE2(r+1),'-g',r,STATISTICS4.ALLIVE2(r+1),'-k')
legend('LEACH   m=0.1,a=2','SEP   m=0.1,a=2','Z-SEP  m=0.1,a=2', 'improved  m=0.8,a=2');
xlabel('Rounds');
ylabel('Allive Nodes');

figure(3);
plot(r,STATISTICS.DEAD3(r+1),'-b',r,STATISTICS.DEAD(r+1),'-r',r,STATISTICS.DEAD2(r+1),'-g',r,STATISTICS4.DEAD2(r+1),'-k')
legend('LEACH  m=0.1,a=2','SEP    m=0.1,a=2','Z-SEP  m=0.1,a=2','improved  m=0.8,a=2');
xlabel('Rounds');
ylabel('Dead Nodes');

figure(4);
plot(r,STATISTICS.packets_TO_BS(r+1),'-r',r,STATISTICS.packets_TO_BS3(r+1),'-b',r,STATISTICS.PACKETS_TO_BS2(r+1),'-g',r,STATISTICS4.PACKETS_TO_BS2(r+1),'-k')
legend('LEACH   m=0.2,a=1','SEP    m=0.2,a=1','Z-SEP m=0.2,a=1','improved  m=0.8,a=2');
xlabel('Rounds');
